...
int main(void)
{
	unsigned int adc_temp,adc_gas;
	unsigned int i;
	float in_vtg,in_vtg1;
	unsigned char vtg[7],dval[7], blank[]="   ";
	unsigned char Msg3[11] = {"TEMP:"};
	unsigned char Msg4[12] = {"gas level:"};

   lcd_init();
   LPC_PINCON->PINSEL3 |= 0xF0000000;																	//P1.31 as AD0.5
...
//		in_vtg = (((float)adc_temp * (float)Ref_Vtg))/((float)Full_Scale);	//calculating input analog voltage
		...
		//in_vtg1 = in_vtg1 - 273.14;	
		sprintf(vtg,"%3.3f",in_vtg1);


//		LPC_ADC->ADCR = (1<<4)|(1<<21)|(1<<24);															//0x01200001;//ADC0.5, start conversion and operational	
//		for(i=0;i<2000;i++);																								//delay for conversion
//		while((adc_gas = LPC_ADC->ADGDR) == 0x80000000);										//wait till 'done' bit is 1, indicates conversion complete
//		adc_gas = LPC_ADC->ADGDR;
//		adc_gas >>= 4;
//		adc_gas &= 0x00000FFF;																							//12 bit ADC
																						//convert the readings into string to display on LCD
		sprintf(dval,"%x",adc_gas);
